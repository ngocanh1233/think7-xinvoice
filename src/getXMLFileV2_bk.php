...
$document
	...
	//	$document
	//	->addDocumentTax("S", "VAT", $value['Gesamt'], $value['Netto'], $value['Steuer']);
	//}
	$document
		->addDocumentTax("S", "VAT", $dFzu_TotalPrice, 483.36, 19.0);
	$document
    ->setDocumentSummation($dFzu_TotalPrice, $dFzu_TotalPrice - $dz_Amount_already_paid , $dFzu_TotalPrice, 0.0, 0.0, $dFzu_TotalPrice, $dFzu_TotalTax, null, $dz_Amount_already_paid)
	...
	foreach($dt_Json as $key => $value){
		...
		->setDocumentPositionGrossPrice($value['Preis'])
		->setDocumentPositionNetPrice($value['Preis'] + $value['Netto'])
		->setDocumentPositionQuantity($value['Menge'], $value['Einheit'])
		->addDocumentPositionTax('S', 'VAT', $value['Steuer'])
		->setDocumentPositionLineSummation($value['Gesamt Netto']);
	}
